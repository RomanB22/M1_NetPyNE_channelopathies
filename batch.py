...
def evolRates():
    ...
    b.evolCfg = {
        ...
        'max_generations': 100,
        'time_sleep': 10 * 300,  # 5min wait this time before checking again if sim is completed (for each generation)
        'maxiter_wait': 5 * 64,  # (5h20) max number of times to check if sim is completed (for each generation)
...
def setRunCfg(b, type='mpi_bulletin', nodes=1, coresPerNode=8):
    if type=='mpi_bulletin':
        ...
        b.runCfg = {'type': 'hpc_sge',
                    ...
                    'cores': 64,
                    'log': os.getcwd()+'/qsub/'+b.batchLabel,
                    'mpiCommand': 'mpiexec',
                    'vmem': '128G',
                    'walltime': "5:00:00",
                    'script': 'init.py',
